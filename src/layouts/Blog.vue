    ...
    <div class="container flex flex-align-top">
      <div class="sidebar" v-if="subtitles">
        <template v-for="subtitle in subtitles">
          <h3 class="menu-item" v-if="subtitle.depth === 2" :key="`title-${subtitle.value}`">
            <a :href="subtitle.anchor">{{ subtitle.value }}</a>
          </h3>
          <a
            :key="`item-${subtitle.value}`"
            v-if="subtitle.depth === 3"
            :href="subtitle.anchor"
          >{{ subtitle.value }}</a>
          <a
            :key="`item-${subtitle.value}`"
            v-if="subtitle.depth === 4"
            :href="subtitle.anchor"
