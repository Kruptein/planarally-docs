...
This will replicate to all other campaigns you play or run, just as if done through the [dashboard](/en/docs/dashboard/#localisation).
...
The colour setting defines in what colour the [grid layer](/en/docs/dm/layers/#grid) (i.e. the grid lines) will be drawn.

### Ruler

Here, the colour of the [ruler](/en/docs/tools/ruler) can be set.
...
When checked, this inverts the default [snapping behaviour](/en/docs/game/snapping/).
...
<Info>
    The two lock settings below are only in use if initiative is active. Initiative is only properly active when toggled
    on using the [Quick Toggles](/en/docs/tools/quick-toggles/).
</Info>
...
This has the same impact as if you used the [vision tool](/en/docs/tools/vision/) and manually changed it to the active initiative participant every turn.
