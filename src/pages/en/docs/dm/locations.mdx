...
The Locations Tool provides multiple instances of a set of [layers](/en/docs/dm/layers/) with their own settings, each instance hereinafter referred to as a _Location_.
...
Players only interact with one canvas at a time; for them, each new Location looks like a different map (i.e. a different set of stacked [layers](/en/docs/dm/layers/)).
...
A newly created Location inherits the Campaign Default settings that are defined in the [settings](/en/docs/dm/settings/).
...
See ['Grid' in the Campaign Settings](/en/docs/dm/settings/#grid).

### Vision

See ['Vision' in the Campaign Settings](/en/docs/dm/settings/#vision).
...
When sent to another location, shapes will appear centered on the chosen spawn location, which is a special shape that appears on the _dm_-[layer](/en/docs/dm/layers/#dm).
