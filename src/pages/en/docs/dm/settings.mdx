...
These settings are the Campaign defaults and can be overridden in the [individual Location Settings](/en/docs/dm/locations/#grid).
...
Most importantly [snapping behaviour](/en/docs/game/snapping) has not been updated with hex support.
...
They are also referenced in the DM section on [lighting and vision](/en/docs/dm/lighting-vision/).

The settings defined in this dialogue are the Campaign defaults that can be overridden in the [individual Location Settings](/en/docs/dm/locations/#vision).

### Fake Player

When this is enabled the screen will be rendered without your DM privileges.
It will instead render using the vision of all the players you've selected in [the vision tool](/en/docs/tools/vision/).
This can help to verify during prep or even in-game that your vision setup is correct.

You can quickly toggle this on/off using [Quick Toggles](/en/docs/tools/quick-toggles/#fake-player)
...
See the [lighting docs](/en/docs/dm/lighting-vision/) for more information on this topic.
...
These settings are tuning factors to the [vision system](/en/docs/dm/lighting-vision/).
