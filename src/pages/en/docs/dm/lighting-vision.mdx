...
_If you want a hands-on example check the [first session tutorial](/en/learn/dm/first-map/#setting-up-a-light)!_
...
For more info see the [auras](/en/docs/game/shapes/#auras) documentation.
...
<Warning>
    This setting will only take into account the assets that at least one player has **vision access** to. If there are
    no tokens a player has vision access to, that player will have **no vision**.
</Warning>

## Minimal/Maximal Vision

These can be set in the [campaign settings](/en/docs/dm/settings/).
