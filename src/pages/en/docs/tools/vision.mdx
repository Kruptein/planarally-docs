...
The vision tool can be accessed in the toolbar on the bottom right of the Game Board in both [modes](/en/docs/tools-overview/#toolbar-and-modes).
It helps to track vision of multiple tokens.
Its behaviour is different for players and DMs.

## Player

If clicked, the Vision tool will open a list of all objects that are [configured with vision access](/en/docs/game/shapes/#access) for the player.
...
They can, however, [fake player](/en/docs/dm/settings/#fake-player) to achieve a similar effect to that described above.
