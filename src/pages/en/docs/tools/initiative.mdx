...
<Image src={InitiativeOverviewPicture} width={462} aspectRatio={462 / 492} alt="Overview of the initiative tool" />
...
<Eye /> toggles vision auto-lock. This function is useful for players who own more than one token. If active, every time
action is assigned to another object (see [next turn](/en/docs/tools/initiative/#next-turn) below), the vision of only
this object will be shown. This can be toggled on/of individually by each player. Vision auto-lock will only display
vision of objects the players have proper access to.

<Info title="Vision Tool">
    Vision Auto-Lock triggers the [Vision tool](/en/docs/tools/vision/) to reconfigure. Use that tool to configure what
