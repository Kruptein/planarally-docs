...
The draw tool can be accessed in the toolbar on the bottom right of the screen by all players, but only in [Build mode](/en/docs/tools-overview/#toolbar-and-modes).
...
<Warning title="Performance">
    Note that drawing with the _paintbrush_ needs to use a lot of points to represent a shape. This is generally only
    advised for small things, as it is pretty bad performance-wise.
</Warning>
...
<Tip title="Edit">You can edit the text afterwards in the shape properties dialog!</Tip>
...
For more information, see [the documentation on magical light and darkness](/en/docs/dm/lighting-vision/#magical-lights-and-darkness).

_Erase_ will create a shape that works as a (portable) 'hole', piercing through all objects _below_ them in the draw stack, including [floors](/en/docs/dm/floors/)!
An _erased_ shape will automatically be dropped on the [map layer](/en/docs/game/gameboard/#map), but can be moved to any layer.

See the [0.26 release notes](/en/blog/release-0.26#erase-floor-mode-dm) for more info on erase mode.
...
See [shape logic](/en/docs/game/shapes/#logic) for more information.
