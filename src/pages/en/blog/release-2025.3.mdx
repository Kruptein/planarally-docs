...
A new config option "admin_user" (See [the config manual](/en/server/management/configuration/#field-admin_user)) is added, that can be configured to designate a specific user as the main admin.
...
In [2024.1](/en/blog/release-2024.1) the "behind" vision blocking mode was introduced.
...
A sneaky syntax bug in the server code was causing assets from other players to show up when searching in the in-game asset manager.

While fixing that bug I also noticed that shared assets where not being queried when searching assets.
It's a more expensive operation to include those by default, so a toggle was added to the search bar to optionally opt-in searching assets that are shared with you.
...
- Floor renaming after a re-order would rename the wrong floor
