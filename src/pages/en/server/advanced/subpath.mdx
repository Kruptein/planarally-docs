...
PlanarAlly can be hosted without effort at a root level (e.g. https://www.planarally.io) or at a subdomain (e.g. https://app.planarally.io).
...
When you want to use a subpath, you have to [build the client yourself](/en/server/setup/self-hosting/#manual-installation).
...
When using nginx, you will have a very similar setup as the one described in [proxy](/en/server/advanced/proxy/), but all `location` parts will start with `/planarally` if that is your desired subpath.
