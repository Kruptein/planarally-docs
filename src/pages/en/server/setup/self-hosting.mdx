...
<br />
Extracting over the existing installation will usually work, but it can sometimes lead to some subtle bugs where a file was removed in the codebase, but will not be removed by overwriting.
<br />
...
See the [config docs](/en/server/management/configuration/) for the details.
...
<Info>
    There is no pre-configured admin user, so when you see the login page you'll have to go through the register flow
    like any other user!

    <br/>

    If you desire you can configure your created user as an admin to see some extra UI by creating/modifying the server config.
...
To build the frontend you need `node` and `npm` installed. At the moment of writing at least version 22 of node is required. (Though I would recommend version 24 if you don't have node installed yet)
...
When you want to (re)start your server in the future you only need to run step 5 (in the server folder).
...
There is also a compose file included in an example setup made for the Traefik reverse proxy, which can be found [here](/en/server/advanced/proxy/#traefik).
...
<Warning>
    The client output will mention a specific port, you should completely ignore this as the PA dev server is hooked up
    in a special manner to be as closely similar to the regular setup.
    <br />
    So you still access PA using the same (server) port as you did before.
</Warning>

<Warning>
    If this is a clean version of the repository, the client must have been build normally once before you can use the
    dev server. Otherwise some files that get generated on the server will be missing.
</Warning>
