...
Information in this guide was heavily sourced from the [Basic Syntax Guide](https://markdownguide.offshoot.io/basic-syntax/)
...
The   `#` character can be used to set title sizes. 1 will give the largest size
...
3 `#` in the next sized down still. 
...
4 `#` is the same as using bold type which can be done either by `**` or `__` surrounding the text. 

```
#### Title Size 4 is the same as **bold**
```
...
For inline images you precede the item with `!` specify a tag name in `[ ]` and then the link in `( )`. 
...
The alternative method, which can make the code cleaner, is a reference method where instead of a link to the image, you can use a reference, which in turn links to the image. References can also supply tool tips so when a cursor hovers over an image a tool tip can be seen. These are made by quoting `" "` the tool tip text after the URL 
