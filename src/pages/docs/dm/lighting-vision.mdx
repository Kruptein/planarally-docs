...
<Warning>
    For performance reasons this setting will only take into account the assets that at least one player has **vision access** to.
    If there are no tokens a player has vision access to, that player will have **no vision**.
</Warning>
