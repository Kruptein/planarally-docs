...
<Info>
    The two lock settings below are only in use if initiative is active. Initiative is only properly active when
    toggled on using the [Quick Toggles](/docs/tools/quick-toggles/).
</Info>
